...
def load_vgg(sess, vgg_path):
    ...
    # Lesson 11: Inference Performance - Reusing The Graph
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # 1x1 convolution for encoder's layers
    ...
    upsample1 = tf.layers.conv2d_transpose(layer_7_1x1, num_classes, 4, 2, padding='same', kernel_initializer= init);
    layer1 = tf.layers.batch_normalization(upsample1)
    layer1 = tf.add(layer1, layer_4_1x1)

    upsample2 = tf.layers.conv2d_transpose(layer1, num_classes, 4, 2, padding='same', kernel_initializer= init);
    layer2 = tf.layers.batch_normalization(upsample2)
    layer2 = tf.add(layer2, layer_3_1x1)

    # return output layer
    return tf.layers.conv2d_transpose(layer2, num_classes, 16, 8, padding='same', kernel_initializer= init);
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch_i in range(epochs):
        for X, y in get_batches_fn(batch_size):
            ...
            #feed_dict={input_image: X, correct_label: y, keep_prob:0.5, learning_rate:0.0001})
            #feed_dict={input_image: X, correct_label: y, keep_prob:0.75, learning_rate:0.001})
            print("Epoch {}/{} - ".format(epoch_i+1, epochs), " loss: {:.4f}...".format(loss))
